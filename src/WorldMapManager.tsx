...
type SelectedHex = { col: number; row: number } | null;
type ViewMode = "scroll" | "drag";
type SettingsTab = "map" | "city";
type PresetMode = "tile" | "building";
type TileStatesByMap = Record<string, Record<string, MapTileStateAssignment[]>>;
type TilePresetsByMap = Record<string, MapTileStatePreset[]>;
type TileRegionStatesByMap = Record<string, Record<string, MapTileRegionState>>;
type BuildingPresetsByMap = Record<string, WorldMapBuildingPresetRow[]>;
type BuildingInstancesByMap = Record<string, WorldMapBuildingInstanceRow[]>;
type ImageViewportBounds = {
  left: number;
  top: number;
  right: number;
  bottom: number;
};
const LEGACY_ANSI_TO_HEX: Record<number, string> = {
  30: "#9ca3af",
  31: "#ef4444",
  32: "#22c55e",
  33: "#eab308",
...
const CAPPED_RESOURCE_IDS = ["wood", "stone", "fabric", "weave", "food"] as const;
const UNCAPPED_RESOURCE_IDS = ["research", "order", "gold"] as const;
const ALL_RESOURCE_IDS = [...CAPPED_RESOURCE_IDS, ...UNCAPPED_RESOURCE_IDS] as const;
const BUILDING_PRESET_RESOURCE_IDS = ALL_RESOURCE_IDS.filter(
  (id) => id !== "research"
) as ResourceId[];
const TRACKED_POPULATION_IDS = ["settlers", "engineers", "scholars", "laborers"] as const;
const ALL_POPULATION_IDS = [...TRACKED_POPULATION_IDS, "elderly"] as const;
const UPKEEP_POPULATION_IDS = [...ALL_POPULATION_IDS, "anyNonElderly"] as const;

type CappedResourceId = (typeof CAPPED_RESOURCE_IDS)[number];
type ResourceId = (typeof ALL_RESOURCE_IDS)[number];
...
const RESOURCE_EMOJIS: Record<ResourceId, string> = {
  wood: "🪵",
  stone: "🪨",
  fabric: "🧵",
  weave: "✨",
...
const POPULATION_LABELS: Record<PopulationId, string> = {
  settlers: "정착민",
  engineers: "기술자",
  scholars: "학자",
  laborers: "역꾼",
...
const UPKEEP_POPULATION_LABELS: Record<UpkeepPopulationId, string> = {
  ...POPULATION_LABELS,
  anyNonElderly: "노약자를 제외한 아무나",
};

const POPULATION_EMOJIS: Record<PopulationId, string> = {
  settlers: "🏠",
  engineers: "🛠️",
  scholars: "📚",
  laborers: "🛺",
...
const EMPTY_STATE_BADGES: Array<{ text: string; color: string }> = [];

type ResourceCostDraft = Partial<Record<ResourceId, string>>;
type PopulationCostDraft = Partial<Record<UpkeepPopulationId, string>>;
type WorkerAssignmentDraft = Record<PopulationTrackedId, string>;

const EMPTY_WORKER_ASSIGNMENT_DRAFT: WorkerAssignmentDraft = {
  settlers: "0",
  engineers: "0",
  scholars: "0",
  laborers: "0",
...
function createDefaultPopulationState(): CityPopulationState {
  return {
...
function createDefaultCityGlobalState(): CityGlobalState {
  return {
    values: {
      wood: 0,
      stone: 0,
...
function normalizeCityGlobalState(input?: Partial<CityGlobalState> | null): CityGlobalState {
...
function buildDraft(map: WorldMap | null): Draft {
...
function pointyHexPoints(cx: number, cy: number, size: number) {
...
function flatHexPoints(cx: number, cy: number, size: number) {
  const points: Array<[number, number]> = [];
  for (let i = 0; i < 6; i += 1) {
...
function legacyAnsiToHex(value: unknown): string | null {
  const n = Number(value);
  if (!Number.isFinite(n)) return null;
  return LEGACY_ANSI_TO_HEX[Math.floor(n)] ?? null;
}

function normalizeHexColor(value: unknown, fallback = "#e5e7eb") {
  return parseHexColor(value) ?? fallback;
}

function normalizeTilePresets(raw: unknown): MapTileStatePreset[] {
...
function normalizeTileStateAssignments(raw: unknown) {
...
function normalizeTileRegionStates(raw: unknown) {
  const src = raw as Record<string, unknown>;
...
function isTrackedPopulationId(value: unknown): value is PopulationTrackedId {
  return typeof value === "string" && TRACKED_POPULATION_IDS.includes(value as PopulationTrackedId);
}

function normalizeRuleExpr(input: unknown, fallback = 0): BuildingRuleExpr {
...
function normalizeExecutionAction(raw: unknown): BuildingRuleAction {
...
function normalizeExecutionRules(raw: unknown, withInterval = false): BuildingExecutionRule[] {
  if (!Array.isArray(raw)) return [];
...
function normalizePlacementRules(raw: unknown): BuildingPlacementRule[] {
  if (!Array.isArray(raw)) return [];
...
function normalizeBuildingPresets(raw: unknown): WorldMapBuildingPresetRow[] {
  if (!Array.isArray(raw)) return [];
...
function normalizeBuildingInstances(raw: unknown): WorldMapBuildingInstanceRow[] {
  if (!Array.isArray(raw)) return [];
...
function readAssignedWorkersFromInstanceMeta(meta: unknown) {
...
function readAssignedWorkersByTypeFromInstanceMeta(
  meta: unknown
...
function sumAssignedWorkersByType(workersByType: Record<PopulationTrackedId, number>) {
  return TRACKED_POPULATION_IDS.reduce((sum, id) => sum + Math.max(0, workersByType[id] ?? 0), 0);
}

function readBuildStatusFromInstanceMeta(meta: unknown): "building" | "active" | null {
  const root = (meta && typeof meta === "object" ? (meta as Record<string, unknown>) : {}) ?? {};
  const buildMeta =
    root.buildMeta && typeof root.buildMeta === "object"
      ? (root.buildMeta as Record<string, unknown>)
...
function getInstanceBuildStatus(
...
function createZeroWorkersByType(): Record<PopulationTrackedId, number> {
  return {
    settlers: 0,
    engineers: 0,
    scholars: 0,
...
function formatDailyDelta(value: number) {
...
function oddRToCube(col: number, row: number) {
...
function hexDistanceByOrientation(
  orientation: HexOrientation,
  c1: number,
  r1: number,
  c2: number,
...
function compareByOp(op: BuildingRuleComparisonOp, left: number, right: number) {
...
function countBuildingsInRange(
  ctx: RuleEvalContext,
...
function evalRuleExpr(ctx: RuleEvalContext, expr: BuildingRuleExpr | undefined): number {
  if (!expr) return 0;
...
function evaluateRulePredicatePreview(
  ctx: RuleEvalContext,
...
function createEmptyResourceCostDraft(): ResourceCostDraft {
  const out: ResourceCostDraft = {};
  for (const id of ALL_RESOURCE_IDS) out[id] = "";
  return out;
}

function createEmptyPopulationCostDraft(): PopulationCostDraft {
  const out: PopulationCostDraft = {};
  for (const id of UPKEEP_POPULATION_IDS) out[id] = "";
  return out;
}

function createDefaultRuleAction(): BuildingRuleAction {
  return {
    kind: "adjustResource",
    resourceId: "gold",
    delta: { kind: "const", value: 0 },
...
function createDefaultExecutionRule(withInterval = false): BuildingExecutionRule {
  return {
    id: makeLocalId(),
...
function createDefaultBuildingDraftState(): BuildingDraftState {
  return {
    id: null,
    name: "",
    color: "#eab308",
...
function toNumberRecordFromDraft<T extends string>(
  ids: readonly T[],
...
function toStringRecordFromNumbers<T extends string>(
  ids: readonly T[],
...
function exprToEditableNumber(expr: BuildingRuleExpr | undefined): string {
  if (!expr) return "";
  if (expr.kind === "const" && Number.isFinite(expr.value)) return String(Math.trunc(expr.value));
  return "";
}

function buildDraftFromPreset(row: WorldMapBuildingPresetRow): BuildingDraftState {
  return {
    id: row.id,
    name: row.name,
